...
import { View, Text, StyleSheet, SafeAreaView, Image, TouchableHighlight, Alert, TextInput, StatusBar, KeyboardAvoidingView, Pressable } from 'react-native';
import { CommonStyles, TextStyles } from '../../../Styles/ComnStyle';
import { height, itemWidth, moderateScale, moderateScaleVertical, scale, textScale, width } from '../../../Styles/responsiveSize';
import { KeyboardAwareScrollView } from 'react-native-keyboard-aware-scroll-view';
import { Alerts, Localize, Titles, En, Constants, AppFonts,ImagePath,Colors } from '../../../Constants';
import * as Components from "../../../Components/indexx"
import Actions from '../../../Redux/Actions';
import { showAlertMessage, success, danger, print, getIconUrl, currencySymbol, calculateSellingCoinsQty, getPriceInPercent, warning, getCountryCurrencyCountryCode } from '../../../Utils/helperFunctions';
...
const UpdateOffer = ({ navigation, route }) => {
    ...
    let PaymentTypeArr = [{ id: 1, paymentMethodType: En.UPI },
    ...
    // const selectLiveCoin = coinDetails ? coinDetails[coin ?? ''] : ''
    // const marketPrice = selectLiveCoin?.inrLivePrice
    ...
    // })
        ...
        // const marketPrice = selectedCoin?.inrLivePrice
        const getMarketPrice = async () => {
            // const [marketPrice, setMarketPrice] = useState(0)
                ...
                // setMarketPrice(MRP)
                    ...
                    // const curretCoin = coin === 'USDT' ? 'tether' : coin?.toLowerCase()
                    // const selectLiveCoin = Object.hasOwn(coinDetails, curretCoin) ? coinDetails[curretCoin ?? ''] : ''
                    // const MRP = Object.hasOwn(selectLiveCoin, OfferDetail?.currency_name.toLowerCase()) ? selectLiveCoin[OfferDetail?.currency_name.toLowerCase()] : selectLiveCoin['usd']
                    // const symb = Object.hasOwn(selectLiveCoin, OfferDetail?.currency_name.toLowerCase()) ? OfferDetail?.currency_symbol : '$'
                    // setMarketPrice(MRP)
                    // setCurrencySymbol(symb)
            }
    ...
    const updateStates = (data) => {
        ...
        setselectedPaymentMetodArr(data?.payment)
        const coin = allCoinsList.find(item => item?.name === data?.coin)
        console.log("selected coin is : --", coin)
        setcoinQty(`${data?.maxQuantity}` || '')
        // setCoinAmount(data?.max?.toString() || '')
        const sellPartially = data?.sell_at_mp?sellPartiallyArr[0]:sellPartiallyArr[1]
        const sell_at_mp = data?.sell_partially?sellTypeArr[0]:sellTypeArr[1]
        onChageCoinQty(data?.maxQuantity)
        setSelectedSellAt(sell_at_mp)
        setIsSellPatially(sellPartially)
        setMarginPercentage(Number(data?.percentage).toString() || '')

        // setMinSellPrice((`${minPrice}`) || '')
        // setMaxSellPrice(`${maxPrice}` || '')
        setTitle(data?.title || '')
    ...
    const updatePaymentMetodArr = (data) => {
        ...
        const isPaymentTypeExist = selectedPaymentMetodArr.some(el => el.id === data?.id);
    ...
    const onPressAmmontPercent = (value) => {
        ...
        let coins = walletCoinDetail.balance
    ...
    const onPressSellOrder = () => {
        ...
        // navigation.navigate(Constants.BuyDeal)

        const isValid = validateCreteOffer(selectedCoin, coinQty, isSellPatially, maxSellPrice, minSellPrice, countryCode, selectedPaymentMetodArr, title, discription)
        {
            (isValid) &&
                setShowModal(true)
        }
    ...
    const onPressContinue = () => {
        ...
        const totlCoinAmnt = coinQty * (marketPrice)
        ...
        const finalmaxPrice = totlCoinAmnt.toFixed(2)
        ...
        const maxCoinsQty = coinQty //calculateSellingCoinsQty({ sellingPrice: finalmaxPrice, marketPrice: marketPrice })
        ...
        const payLoad = {
            ...
            title: title,
            payment: selectedPaymentMetodArr.map((item) => item.paymentMethodType),//?.replace(/ /g, ""),.replace(/ /g, "")
            country: countryCode,
            selling_price: coinAmount,
            quantity: coinQty,
            coin: selectedCoin?.name,

            maxQuantity: maxCoinsQty,
            minQuantity: minCoinsQty,
            maxAmount: finalmaxPrice,
            minAmount: finalminPrice,
    ...
    const clearData = () => {
        ...
        if (coinsTypeArr.length > 0) {
            updateCoinSelection(coinsTypeArr[0])
        } else {
            updateCoinSelection([])
        }
    ...
    const onChageCoinQty = (val) => {
        ...
        // if (val <= walletCoinDetail?.balance) {
        ...
        const coinAmunt = val * (marketPrice)
        ...
        if (val <= walletCoinDetail?.balance) {
            ...
            const err = { ...error, coinQty: `You not have enough coins you only have create (${(walletCoinDetail?.balance)} ${selectedCoin?.name}) deal` }
            setError(err)
    ...
    const onChangeAmunt = (val) => {
        const coinAmunt = val * (marketPrice ?? 0)
        setMaxSellPrice(coinAmunt.toFixed(2).toString())
        const minPric = parseFloat(coinAmunt) / 100 * parseFloat(10)

        setMinSellPrice(minPric.toFixed(2).toString())
        ...
        const minPr = (parseFloat(finalPrice) / 100 * parseFloat(10)).toFixed(2)//getPriceInPercent(finalPrice, 10)
        const calQty = parseFloat(minPr) / parseFloat(sellPerCoin)
        setMinQtySell(calQty.toFixed(8).replace(/\.0+$/, "").toString())
    ...
    const onChangePercent = (val) => {
        ...
        const marginPrice = (Number(coinsAmnt) / 100 * Number(val)).toFixed(2)
        let finalPrice = (Number(coinsAmnt) + Number(marginPrice)).toFixed(2)
        ...
        if (val) {
            ...
            setMaxSellPrice(Number(finalPrice).toFixed(2).toString())
            const sellPerCoin = Number(finalPrice) / Number(coinQty)
            setCoinSellAt(sellPerCoin)
            const minPric = (parseFloat(finalPrice) / 100 * parseFloat(10)).toFixed(2)//getPriceInPercent(finalPrice, 10)
            setMinSellPrice(minPric && minPric.toString())
            setMinQtySell(parseFloat(val) / parseFloat(sellPerCoin))


        } else {
            setMaxSellPrice(coinsAmnt.toFixed(2).toString())
            const minPric = (parseFloat(coinsAmnt) / 100 * parseFloat(10))
            setMinSellPrice(minPric && minPric.toFixed(2).toString())
            // const sellPerCoin = parseFloat(finalPrice)/parseFloat(coinQty)
            setCoinSellAt(marginPrice)
            setMinQtySell(parseFloat(minPric) / parseFloat(marketPrice))
    ...
    const validateCreteOffer = (selectedCoin, CoinQty, isSellPatially, maxSellPrice, minSellPrice, country, payment, title, discription,) => {
        if (!selectedCoin) {
        ...
        } else if (CoinQty > walletCoinDetail?.balance) {
            showAlertMessage("Please enter valid coin Qty")
            return false
        }
            ...
            <View style={styles.amountfromWalletContainer}>
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: moderateScaleVertical(16) }}>
                    {PaymentTypeArr.map((item, index) => {
                        const isSelected = selectedPaymentMetodArr?.filter(data => data.id == item?.id)
                        return (
                            <TouchableHighlight //>
                                underlayColor={Colors.transparent}
                                onPress={() =>
            ...
            <View style={styles.amountfromWalletContainer}>
                ...
                >
                    <Pressable
                        ...
                        style={{ alignItems: 'center', justifyContent: 'center', paddingHorizontal: moderateScale(10) }}
                        ...
                        onPress={() => {
                            ...
                            } else if (coinQty <= 0 || parseFloat(coinQty) > parseFloat(walletCoinDetail?.balance)) {
                    ...
                    <TextInput
                        ...
                        editable={coinQty != '' || (coinQty > 0 && coinQty <= walletCoinDetail?.balance)}
                ...
                <Text style={{ ...TextStyles.medium, ...styles.orderDiscript, }}>{`${En.Market_price}:  ${currencySymbol}${marketPrice}`}</Text>
    ...
    const TitleMessage = () => {
        ...
        // ((maxSellPrice) && maxSellPrice > (coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")) ?
        // <Text style={{ ...TextStyles.medium, ...styles.error, color: Colors.text_Green_dark }}>{`You have sell this deal ( ${currencySymbol('INR')} ${(maxSellPrice - coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")} ) above market price`}</Text>
        // : (maxSellPrice && maxSellPrice < (coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")) ?
        //     <Text style={{ ...TextStyles.medium, ...styles.error, color: Colors.text_red_dark }}>{`You have sell this deal ( ${currencySymbol('INR')} ${(coinQty * marketPrice - maxSellPrice).toFixed(2).replace(/\.0+$/, "")} ) below market price`}</Text>
        //     : (maxSellPrice && coinQty != '' && (coinQty > 0 && (coinQty <= walletCoinDetail?.balance))) &&
        //     <Text style={{ ...TextStyles.medium, ...styles.error, color: Colors.text_Yellow }}>{`You have sell this deal as market price`}</Text>

        const marginPrice = ((maxSellPrice) && maxSellPrice > (coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")) ? <Text style={{ ...TextStyles.small, }}> You have sell this deal <Text style={styles.higlighText}>{`( ${currencySymbol} ${(maxSellPrice - coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")} )`}</Text> above market price.</Text> :
            (maxSellPrice && maxSellPrice < (coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")) ? <Text style={{ ...TextStyles.small, }} > You have sell this deal <Text style={styles.higlighText}>{`( ${currencySymbol} ${(maxSellPrice - coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")} )`}</Text>  below market price.</Text> :
        ...
        return (
            ...
            <>
                <Text style={{ ...TextStyles.small, textAlign: 'center' }}>
                    You are creating the deal of<Text style={styles.higlighText}> {coinQty ? parseFloat(coinQty).toFixed(8).replace(/\.0+$/, "").toString() : 0} {selectedCoin?.name.charAt(0).toUpperCase() + selectedCoin?.name.slice(1)}</Text> at price of<Text style={styles.higlighText}> {`(${currencySymbol} ${maxSellPrice})`}</Text>{marginPrice}{mpText}.
                </Text>
            </>
    ...
    return (
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: Colors.app_White }}>
            ...
            <Components.CustomPopUp
                ...
                renderSubtitle={<TitleMessage />}
                // renderSubtitle={<TitleMessage/>}
                // AlertMessageTitle={Alerts.ARE_YOU_WANT_TO_CREATE_OFFER}
                ...
                <View style={styles.container}>
                    <View style={styles.innerContainer}>
                        ...
                        <TextInputVw
                            ...
                            bottomText={`${En.Available_Amount} ${walletCoinDetail?.balance ?? ''}`}
                            value={coinQty}
                        ...
                        {(isSellPatially.id == 1) ?
                            ...
                            <View style={{ flexDirection: 'row', flexGrow: 2, gap: 20, }}>
                                <TextInputVw
                                    containerStyle={{ marginBottom: moderateScale(!error ? 12 : 0), marginTop: moderateScaleVertical(20), flex: 2 }}
                                    headerText={En.Min_Ammount}
                                    bottomText={(minSellPrice && minSellPrice > 0) && `Min coin Qty is ${parseFloat(minQtySell).toFixed(6).replace(/\.0+$/, "")}`}
                                    ...
                                    onChangeText={(val) => {
                                        const maxLimit = (maxSellPrice / 100 * 80).toFixed(2)
                                        setMinSellPrice(val)
                                ...
                                <TextInputVw
                                    ...
                                    onChangeText={(val) => {
                                        ...
                                        setMinSellPrice(minPrice.toFixed(2).toString())
                                        setMaxSellPrice(val)
                                        onChageCoinQty(coins)
                                        if (coins <= blnc) {
                                            setError('')
                                        } else {
                                            const err = { ...error, maxAmount: `You not have enough balance to create maximum ${currencySymbol} ${(walletCoinDetail?.balance * marketPrice).toFixed(2)} deal` }
                                            setError(err)
                                            console.log(err)
                                        }
                            ...
                            <>
                                <TextInputVw
                                    containerStyle={{ marginBottom: moderateScale(!error ? 12 : 0), marginTop: moderateScaleVertical(20), }}
                        ...
                        {((error?.maxAmount || error?.minAmount)) ? <></> :
                            ...
                            ((maxSellPrice) && maxSellPrice > (coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")) ?
                                <Text style={{ ...TextStyles.medium, ...styles.error, color: Colors.text_Green_dark }}>{`You have sell this deal ( ${currencySymbol} ${(maxSellPrice - coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")} ) above market price`}</Text>
                                : (maxSellPrice && maxSellPrice < (coinQty * marketPrice).toFixed(2).replace(/\.0+$/, "")) ?
                                    <Text style={{ ...TextStyles.medium, ...styles.error, color: Colors.text_red_dark }}>{`You have sell this deal ( ${currencySymbol} ${(coinQty * marketPrice - maxSellPrice).toFixed(2).replace(/\.0+$/, "")} ) below market price`}</Text>
                                    : (maxSellPrice && coinQty != '' && (coinQty > 0 && (coinQty <= walletCoinDetail?.balance))) &&
                                    <Text style={{ ...TextStyles.medium, ...styles.error, color: Colors.text_Yellow }}>{`You have sell this deal as market price`}</Text>
                        }
                        {countyPickerModal()}
