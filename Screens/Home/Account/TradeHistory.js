...
const TradeHistory = ({ navigation }) => {
  ...
  const getTransactionList = () => {
    ...
    let query = `?from_date=${applyFilters?.dateRange?.from ?? ''}&to_date=${applyFilters?.dateRange?.to ?? ''}&offer_status=${offerStatus.toString() ?? ''}&offer_crypto=${cryptos.toString() ?? ''}&pageNumber=${currentPageNo}&pageLimit=${10}`;
    console.log("all chat users list query", query);
    Actions.geMyTradeHistory(query)
      .then((res) => {
      ...
      <View style={{ ...styles.mainContainer }}>
        <View style={{ ...styles.innerContainer }}>
          ...
          <Components.BackBtnHeader
            navigation={navigation}
            isSowText={false}
            centerheaderTitle={`${En.Offer_History}`}//lastScreenData?.name
            isRightBtn={true}
            rightBtnAction={onpressFilter}

            isBadgeEnable={applyFilters?.dateRange?.from || applyFilters?.dateRange?.to || applyFilters?.transactionStatus.length > 0 || applyFilters?.cryptoType.length > 0
            }
          />
