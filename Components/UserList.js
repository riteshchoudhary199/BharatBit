...
import { currencySymbol, formatNumberToFixed, showAlertMessage } from '../Utils/helperFunctions';

// create a component
const UserList = ({ data, containerStyle, navigation, }) => {

    const myDeatils = useSelector((state) => state.auth?.userDetail)
    // console.log('myDeatils in user list ', myDeatils)
    ...
    let offrCreateDate = moment(lastChat?.createdAt).format('Do, MMM YYYY')

    const offerStatusText = () => {
        const isCanceled = data?.isCanceled
        let status = data?.chatStatus
        ...
        return (
            <Text numberOfLines={2} ellipsizeMode='tail' style={{
                ...TextStyles.bold,fontSize:16 ,color: status == 'pending' ?
                    Colors.text_Yellow : status == 'successful' ? Colors.text_green : status =='canceled'? Colors.text_red_dark:status == 'disputed' ?Colors.text_Yellow:Colors.text_Black
            }}>
                        ...
                        <View style={{ alignItems: 'flex-end',flexShrink:1 }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: moderateScale(2),flexShrink:1 }}>
                                <Text numberOfLines={2} ellipsizeMode='tail' style={{ ...TextStyles.bold, fontSize: textScale(15) ,}}>
                                    {/* {"15 BTC"} */}
                                    {`  ${(data?.sellingQuantity ? formatNumberToFixed(data?.sellingQuantity):'' )} ${(offer_details?.symbol || '').toUpperCase()}`}
                                </Text>
                                <Components.LoaderImage
                            ...
                            <Text numberOfLines={2} ellipsizeMode='tail' style={{ ...TextStyles.medium, textAlign: 'right', color: Colors.text_green, fontSize: textScale(14) }}>
                                {`${currencySymbol('INR')} ${formatNumberToFixed(data?.sellingAmount,2)|| ''}`}
                            </Text>
                            {(data?.unread_count !== 0) &&
